...
class ChangeStatus(commands.Cog):
    def __init__(self, bot, filepath="quotes.txt"):
        self.bot = bot
        self.quotes = self.load_quotes(filepath)
        self.status.start()
    ...
    async def status(self):
        ...
        await self.bot.change_presence(status=discord.Status, activity=discord.Game(random_quote))

async def setup(bot):
    filepath = "./cogs/quotes_folder/quotes.txt"
    await bot.add_cog(ChangeStatus(bot, filepath))
...
#     ]  
#         # random_quote = random.choice(self.quotes)
#         await self.bot.change_presence(status=discord.Status, activity=discord.Game(random.choice(quotes)))

# async def setup(bot):
#     await bot.add_cog(change_status(bot))